export const UPDATE_SEARCH_INPUT = "UPDATE_SEARCH_INPUT";
export const UPDATE_SEARCH_RESULT = "UPDATE_SEARCH_RESULT";
export const SHOW_SEARCH_PAGE = "SHOW_SEARCH_PAGE";
export const HIDE_SEARCH_PAGE = "HIDE_SEARCH_PAGE";
export const START_SEARCH = "START_SEARCH";
export const UPDATE_HISTORY_RECORD = "UPDATE_HISTORY_RECORD";
export const SHOW_USER_INFO = "SHOW_USER_INFO";
export const HIDE_USER_INFO = "HIDE_USER_INFO";
export const ADD_NEWS_BEFORE_EXSITING = "ADD_NEWS_BEFORE_EXSITING";
export const ADD_NEWS_AFTER_EXSITING = "ADD_NEWS_AFTER_EXSITING";
export const DELETE_NEWS = "DELETE_NEWS";
export const SHOW_BROWSER_MODAL = "SHOW_BROWSER_MODAL";
export const HIDE_BROWSER_MODAL = "HIDE_BROWSER_MODAL";
