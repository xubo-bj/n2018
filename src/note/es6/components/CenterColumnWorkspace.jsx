...
class CenterColumnWorkspace extends React.Component {
	...
	componentDidUpdate() {
		...
		(function scrollToNewFile() {
			let newFileElem = document.querySelector("[data-id=tempId]");
			if (newFileElem != null) {
				let body_bottom = document.body.getBoundingClientRect().bottom;
				let bottom = newFileElem.getBoundingClientRect().bottom;
		...
		return (
			...
			>
							...
							if (dir.editable) {
								...
								return (
									...
									>
										...
										<span className={styles["dir-mtime"]}>
							...
							if (file.editable != true) {
								return (
									...
									>
										...
										<span className={styles["file-mtime"]}>
				...
				>
					...
					<li className={styles["menu-option"]} onClick={deleleFile}>
						删除
					</li>
				</ul>
				<div
		...
		dispatch((dispatch, getState) => {
			...
			if (selectedFileId == fileId) {
				...
				if (filesObj[selectedFileId] != undefined) {
					dispatch(
						get_file_from_local(
							filesObj[selectedFileId].content,
							selectedFileId,
							filesObj[selectedFileId].name
						)
					);
				} else {
	...
	showFileMenu: e => {
		...
		dispatch((dispatch, getState) => {
			...
			if (document.body.clientHeight - e.clientY > 60) {
				dispatch(
					show_center_file_menu(e.clientX, e.clientY, selectedFileId)
				);
			} else {
				dispatch(
					show_center_file_menu(
	...
	deleleFile: e => {
		dispatch((dispatch, getState) => {
			...
			if (fileId == fileIdInProcessing) {
				...
				let afterDeleteFiles = files.filter(
					file => file._id != fileIdInProcessing
				);
