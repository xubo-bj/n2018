...
class DirTree extends React.Component {
	...
	componentDidUpdate() {
		(function renameFolderSelection() {
			if (
				this.props.newFolderRef != null &&
				this.$isEditingFolderNmae != null
			) {
		...
		(function() {
			if (
				...
				let body_bottom = document.body.getBoundingClientRect().bottom;
				let bottom = this.props.newFolderRef.current.getBoundingClientRect()
					.bottom;
				if (bottom > body_bottom) {
					let wrapper = document.querySelector("[data-wrapper-left]");
					wrapper.scrollTop = bottom - body_bottom + 30;
				}
			}
		}.bind(this)());
	}
						...
						if (!dir.editable) {
							...
							return (
								...
								>
									<div
										className={
											centerColumnDir == dir._id
												? styles["li-content-selected"]
												: styles["li-content"]
										}
										style={{
									...
									>
										<i
											className={
												childTargetDir.dirs.length == 0
													? styles["arrow-hidden"]
													: childTargetDir.folded
													? styles["arrow-closed"]
													: styles["arrow-open"]
											}
											onClick={
										...
										<i
											className={styles["arrow-menu"]}
											data-mark="arrow-menu"
										/>
									</div>
									<DirTree
										tree={tree}
								...
								>
									...
									>
										...
										<div className={styles.dir}>
											<i
												className={styles["dir-closed"]}
											/>
											<span
												className={styles.dirName}
												onKeyDown={this.keydown}
												onClick={
													this.clickInEditingFolder
												}
												ref={newFolderRef}
...
class LeftColumnWorkspace extends React.Component {
	...
	componentDidUpdate() {
		(function updateScollbar() {
			let wrapperHeight = this.$wrapper.offsetHeight,
				...
				scrollbarHeight = Math.round(
					(wrapperHeight * wrapperHeight) / wrapperScrollHeight
				);
			this.$scrollbar.style.height = scrollbarHeight + "px";
			this.$scrollbar.style.top =
				Math.round(
					(wrapperScrollTop * wrapperHeight) / wrapperScrollHeight +
						wrapperScrollTop
				) + "px";
		...
		console.log("body-top", document.body.getBoundingClientRect().bottom);
		console.log("wrapper", this.$wrapper.getBoundingClientRect().bottom);
	}
	render() {
		const {
			tree,
		...
		return (
			<div className={styles.workspace}>
				<div
					data-id={shinelonId}
					className={
						centerColumnDir == shinelonId
							? styles["my-dir-selected"]
							: styles["my-dir"]
					}
				...
				>
					<li
						...
						onClick={createNewFilePrompt_2}
					>
						新建笔记
					</li>
					<li
						className={styles["menu-option"]}
						onClick={createNewFolderPromptInRoot}
					>
						新建文件夹
					</li>
				</ul>
			...
			if (target.dataset.mark == "arrow-menu") {
				...
				if (files.length == 0) {
					...
					if (filesObj[fileId] != undefined) {
						dispatch(
							get_file_from_local(
								filesObj[fileId].content,
								fileId,
								filesObj[fileId].name
							)
						);
					} else {
